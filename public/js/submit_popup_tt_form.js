...
$('#submitParameterForm').on('submit', function(e) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var submitFormId = $("#submit_parameter_id").val();
            var formType = $("#formType").val();
            var tt_button_id = $("#tt_button_id").val();
            var TilesFId = $("#TilesFId").val();
            // missing-values
            if(submitFormId == '') {
                var bulkUpdateIds = $("#bulk_update_id").val();
                jQuery.each(bulkUpdateIds.split(','), function(index, item) {
                    $(".ttformSubmit" + item).html('<span style="font-weight: bold;">Product Area:</span><span>' + pa + ',</span><span style="font-weight: bold;">Issue Type:</span><span>' + it + ',</span><span style="font-weight: bold;">Product Family:</span><span>' + pf + ',</span><span style="font-weight: bold;">Product Component:</span><span>' + pc + '</span>');
                    if(pa == '' || it == '' || pf == '' || pc == '') {
                        $(".ttformSubmit" + item).closest("tr").addClass("missing-values");
                    } else {
                        $(".ttformSubmit" + item).closest("tr").removeClass("missing-values");
                    }              
                });
            } else {
                $(".ttformSubmit" + submitFormId).html('<span style="font-weight: bold;">Product Area:</span><span>' + pa + ',</span><span style="font-weight: bold;">Issue Type:</span><span>' + it + ',</span><span style="font-weight: bold;">Product Family:</span><span>' + pf + ',</span><span style="font-weight: bold;">Product Component:</span><span>' + pc + '</span>');
                if(pa == '' && it == '' && pf == '' && pc == '') {
                    $(".ttformSubmit" + submitFormId).closest("tr").addClass("missing-values");
                } else {
                    $(".ttformSubmit" + submitFormId).closest("tr").removeClass("missing-values");
                }
            }
            //}
