        ...
        <tbody>
            @foreach(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $index => $day)
                <tr class="{{ $day }}-row">
                    ...
                    <td>{{ ucfirst($day) }}</td>
                    <td><input class="timepicker" type="text" id="{{ $day . '_start_time_0' }}" name="{{ $day }}[start_time][]" value="{{ old($day)['start_time'][0] ?? null }}" placeholder="Add Time" {{ old('day_'.$index) ? '' : 'disabled' }}><div></td>
                    <td>to</td>
                    <td><input class="timepicker" type="text" id="{{ $day . '_end_time_0' }}" name="{{ $day }}[end_time][]" value="{{ old($day)['end_time'][0] ?? null }}" placeholder="Add Time" {{ old('day_'.$index) ? '' : 'disabled' }}></td>
