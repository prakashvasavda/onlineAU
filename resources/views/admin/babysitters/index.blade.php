                ...
                <div class="col-12">
                    <div class="card card-secondary">
                        ...
                        <div class="card-body table-responsive">
                            <table id="babysittersTable" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        <th>Name</th>
                                        <th>Email</th>
...
<script type="text/javascript">
    $(function () {
        var table = $('#babysittersTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.candidates.babysitters') }}",
            columns: [
                {data: 'name', name: 'name'},
    ...
    function deleteCandidate(user_id, role){
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    url: "{{url('admin/candidates/delete-babysitters')}}/"+user_id,
                    type: "DELETE",
                    data: {_token: '{{csrf_token()}}' },
                    success: function(response){
                        if(response.status == 200){
                            $('#babysittersTable').DataTable().ajax.reload();
