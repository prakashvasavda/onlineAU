...
class AupairsController extends Controller{
    ...
    public function view_aupair_candidates(Request $request){
        $data['menu']   = "aupairs";
        $aupairs       = FrontUser::where('role', 'au-pairs')->get();
        
        if($request->ajax()){
            return DataTables::of($aupairs)
                ->addColumn('action', function ($row) {
                    $btn = '<span data-toggle="tooltip" title="Edit Candidate" data-trigger="hover">
                                <a href="'.url('admin/candidates/edit-aupairs/'.$row->id).'" class="btn btn-sm btn-primary edit-candidate" type="button" data-id="' . $row->id . '"><i class="fa fa-pen"></i></a>
                            </span>';
        ...
        return view('admin.aupairs.index', $data);
    }
    ...
    public function update_aupair_candidate(Request $request, $id){
        $request->validate([
            'name'                  => 'required',
            'age'                   => 'required',
            'id_number'             => "required",
    ...
    public function delete_aupair_candidate(Request $request, $id){
